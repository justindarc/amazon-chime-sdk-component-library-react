...
let appInstanceArn = 'arn:aws:chime:us-east-1:946724017157:app-instance/e5d0c6f6-5ce8-4864-91b6-f0fd2ceb71dd';
let serverAdminUserId = 'ServerAdmin';
...
server.listen(port, hostname, () => {
  var describeAppInstanceAdminParams = {
    AppInstanceArn: appInstanceArn,
    AppInstanceAdminArn: serverAdminArn
  };
  
  chime.describeAppInstanceAdmin(describeAppInstanceAdminParams, function(err, data) {
    if (err) log(err); // an error occurred
    else if (!data.AppInstanceAdmin) {
      var createAppInstanceUserParams = {
        AppInstanceArn: appInstanceArn,
        AppInstanceUserId: serverAdminUserId,
        ClientRequestToken: '951c7a68-f9a5-41ae-856f-f68fbc59b493',
        Name: serverAdminUserId
      };

      chime.createAppInstanceUser(createAppInstanceUserParams, function(err, data) {
        if (err) log(err); // an error occurred
        else { 
          var createAppInstanceAdminParams = {
            AppInstanceArn: appInstanceArn,
            AppInstanceAdminArn: serverAdminArn
          };
        
          chime.createAppInstanceAdmin(createAppInstanceAdminParams);
        }
      });
    }
  });
